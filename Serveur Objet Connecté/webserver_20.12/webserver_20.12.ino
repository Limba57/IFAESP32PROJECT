...
void setup() {
  ...
  Stepper monMoteur(nombreDePas,27,33,32,14);
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            } else if (header.indexOf("GET /Servo/on") >= 0) {
              Serial.println("GPIO Servo on");
              monMoteur.step(100);
            } else if (header.indexOf("GET /Servo/off") >= 0) {
              Serial.println("GPIO Servo off");
              monMoteur.step(-100); 
            }
            ...
            // Display current state, and ON/OFF buttons for servo  
            client.println("<p>GPIO 13 - State " + output13State + "</p>");
            // If the output13State is off, it displays the ON button       
            if (output13State=="off") {
              client.println("<p><a href=\"/13/on\"><button class=\"button\">ON</button></a></p>");
